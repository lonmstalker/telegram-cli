...
## [2026-07-19] accepted | D-20260719-002 | Leave требует ordered membership proof, pending join не является членством

- Context: owner-requested leave → join нельзя считать завершённым по одному успешному response
  `leaveChat`/`joinChatByInviteLink`: TDLib отдельно публикует membership state, а invite может
  вернуть запрос на одобрение.
- Decision: typed `leave_chat` считается terminal только при более новом reducer observation со
  status `left`/`banned`; уже наблюдаемый not-member даёт idempotent `already_not_member` без
  dispatch. Timeout возвращает `uncertain` и запрещает blind retry. `ensure_membership`
  сохраняет `request_pending`/`approval_required` как partial и не подменяет их member proof.
- Policy: `leaveChat` reviewed как `reversible_mutation/reconcile`; caller обязан иметь scoped
  lease, generated method risk остаётся source of truth. Известный chat ID не используется для
  обхода invite approval.
- Evidence: [`chat.rs`](../../crates/telegram-core/src/workflows/chat.rs),
  [`runtime.rs`](../../crates/telegram-core/src/runtime.rs),
  [workflow contract](../../docs/chat-resolution-membership.md) и
  [sanitized live checkpoint](../raw/2026-07-19-p10-chat-membership-roundtrip.md).
- Consequences: live CHAT-010 может честно остановиться после доказанного leave и отправленной
  заявки; acceptance требует последующего terminal member state с тем же chat ID.

## [2026-07-19] correction | D-20260719-002 | Pending завершает submission, membership подтверждается отдельным status

- Context: исходная запись называла `request_pending` partial целиком, хотя TDLib уже terminal
  подтвердил отправку заявки; неопределённым остаётся membership, а не submission.
- Correction: `ensure_membership` возвращает allowlisted receipt с отдельными
  `submission_complete` и `membership_complete`. `request_pending` даёт root `status=ok`,
  `submission_complete=true`, `membership_complete=false`; raw `ChatJoinResult` удалён из output.
- Observation: новый read-only `membership_status` по chat ID или invite link выполняет fresh
  group probe, применяет preceding TDLib updates через response boundary и отдельно доказывает
  `member/not_member/banned`. Unresolved/unknown остаются incomplete без повторного join.
- Evidence: [design](../../docs/brainstorms/2026-07-19-async-membership-status.md),
  [`chat.rs`](../../crates/telegram-core/src/workflows/chat.rs),
  [`runtime.rs`](../../crates/telegram-core/src/runtime.rs) и
  [sanitized live status](../raw/2026-07-19-p10-chat-async-membership-status.md).
- Consequences: caller может завершить команду сразу после `request_pending`, а позже poll/watch
  безопасно получает membership; mutation journal не применяется к status workflow.

## [2026-07-21] accepted | D-20260721-001 | Open chat cleanup получает отдельное bounded окно

- Context: `raw_api` dispatch-ит TDLib request до ожидания response. Поэтому timeout ответа
  `openChat` не доказывает, что open не применён; прежний lease создавался только после ACK.
  Кроме того, общий absolute workflow deadline уже был исчерпан после full-info timeout и не
  позволял получить correlated `closeChat` acknowledgement.
- Decision: timeout ответа `openChat` вызывает ровно один exact compensating `closeChat`.
  Explicit close и `Drop` fallback используют `max(workflow_deadline, now + 4s)`. Четыре секунды
  сохраняют bounded cleanup и оставляют запас внутри 35-second CLI timeout после 30-second daemon
  workflow boundary.
- Retry boundary: timeout самого `closeChat` остаётся uncertain. Lease disarm-ится до explicit
  close, поэтому `Drop` не выполняет blind second close без observable desired-state probe.
- Evidence: [`chat.rs`](../../crates/telegram-core/src/workflows/chat.rs),
  [`runtime.rs`](../../crates/telegram-core/src/runtime.rs),
