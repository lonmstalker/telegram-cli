...
`user_profile`, `update_profile_name`, `plan_chat_title`, `apply_chat_title`, `resolve_chat`, `preview_invite_link`, `ensure_membership`, `load_chat_list`, `inspect_chat`,
...
`resource_statistics`,
`proxy_status`, `set_proxy_enabled`,
`download_file`, `cancel_download`, `upload_sticker_file`, `start_bot`,
`plan_custom_emoji_set`, `apply_custom_emoji_set`,
`plan_story_mutation`, `apply_story_mutation`, `inspect_group_call`, `leave_group_call`,
`notification_settings`, `set_notification_settings`, `active_sessions`,
`plan_terminate_session`, `apply_terminate_session`,
`business_connection`, `send_business_text`,
`star_balance`, `plan_star_invoice_payment`, `apply_star_invoice_payment`,
...
`apply_terminate_session`. Receipt проверяется общим daemon verifier и расходуется внутри
matching TDLib request, а не по имени workflow.

Основные input shapes следуют core types: public chat target — tagged `kind` с `chat_id`,
`username` или public `url`; invite preview — отдельный `{ "url": "..." }`; pagination — nested `page { count, min_date, page_limit }`; file
...
Story/call boundary описана в [`story-call-workflow.md`](story-call-workflow.md): story
post/delete имеют exact approval и server reread, group-call join payload остаётся только у
будущего tgcalls adapter, а CLI умеет безопасно inspect/leave уже существующий call.

Notification/session desired state описан в
[`account-settings-workflow.md`](account-settings-workflow.md); password/recovery secrets не
принимаются ordinary workflow input.

Business connection isolation описана в [`business-workflow.md`](business-workflow.md): оба
route требуют exact connection ID, а uncertain send никогда не повторяется.

Stars-only financial boundary описана в
[`stars-payment-workflow.md`](stars-payment-workflow.md): amount/seller входят в exact plan,
credentials и verification URL не попадают в model-visible transport.

`chat_statistics` и redacted read-only `resource_statistics` описаны в
[`members-statistics-workflow.md`](members-statistics-workflow.md); optimization никогда не
является скрытым side effect чтения.

Platform/proxy boundary описана в
[`platform-utilities-workflow.md`](platform-utilities-workflow.md): endpoints redacted,
setter работает только с existing ID и требует ordered Ready proof.
