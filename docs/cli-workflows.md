...
`apply_chat_title`, `apply_custom_emoji_set` и `apply_story_mutation`. Receipt проверяется
общим daemon verifier и расходуется внутри matching TDLib request, а не по имени workflow.
