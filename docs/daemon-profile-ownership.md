...
После успешного lock daemon поднимает private profile socket по [socket/election contract](daemon-profile-socket.md), но прямо сообщает, что service protocol ещё не реализован. TDLib runtime/profile key wiring, leases и lifecycle пока не запускаются.
