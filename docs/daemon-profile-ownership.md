...
После успешного lock daemon удерживает process, но прямо сообщает, что service transport ещё не реализован. Unix socket, atomic startup election и stale-socket recovery принадлежат следующему Tasks-пункту; TDLib runtime/profile key wiring и lifecycle также пока не запускаются.
...
- Process-level synthetic gate запускает два `telegramd` на одной temporary DB directory: второй завершается с `AlreadyOwned`; после process exit replacement owner получает lock. Реальная TDLib DB не используется.
