...
- SRC003: plans.md, `telegramd::{ownership,socket,lease,server,scheduler}` и `telegram-protocol`; type: file/code; supports: ownership/election, leases и fair read/mutation admission; limits: TDLib dispatch/lifecycle runtime отсутствуют.
...
Account/session model принят до runtime в `D-20260715-036`. Свежий protected P1 live gate proves encrypted returning session can reach Ready, `getMe` and Closed без нового login input. P2 process gates доказывают ownership/socket/lease path; deterministic threaded gate доказывает bounded reads, mutation serialization и FIFO no-overtake. Client autostart convergence, TDLib runtime wiring и idle restart остаются недоказанными.
...
- Partial: canonical owner/socket, local leases и scheduler admission реализованы; client convergence, TDLib dispatch и lifecycle runtime отсутствуют.
