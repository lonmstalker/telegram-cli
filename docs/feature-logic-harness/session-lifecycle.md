...
- SRC003: plans.md, `telegramd::{config,identity,lifecycle,ownership,socket,lease,server,scheduler}` и `telegram-protocol`; type: file/code; supports: ownership/election, returning runtime, identity, leases, idle close и fair read/mutation admission; limits: generic TDLib dispatch и CLI lazy-start отсутствуют.
...
Account/session model принят до runtime в `D-20260715-036`. Protected P2 live gates подтверждают encrypted returning `Ready/getMe`, normal `close -> authorizationStateClosed`, client-crash TTL, daemon-crash restart без login challenge и одного winner для concurrent starts/clients. Deterministic threaded gate доказывает bounded reads, mutation serialization и FIFO no-overtake. CLI-команда lazy start остаётся P6 surface, не P2 runtime claim.
...
- Local default — 30 seconds; server/resident deployment tuning остаётся открытым в Q001.
...
- Writes: TDLib DB state и owner-only profile identity marker; leases/lifecycle state in-memory.
...
- Implemented kernel: canonical owner/socket, local leases, fair admission, returning TDLib runtime, stable profile identity, concurrent-start convergence, TTL expiry и graceful idle close.
- Partial feature surface: generic TDLib dispatch, CLI lazy-start/status и resident/scheduled packaging принадлежат P3–P9.
