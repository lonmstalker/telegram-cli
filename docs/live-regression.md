...
## CHAT-006 — `openChat`/`closeChat` pairing

Статус: `passed` (2026-07-21).

Цель: доказать, что scoped presence lease открывает доступный chat только на время inspection и
получает correlated `closeChat` acknowledgement при success; timeout/error paths не оставляют
open lifetime без компенсирующего cleanup.

Воспроизведение:

```sh
# Отдельный daemon process с минимальным owner ceiling; .env.local не меняется.
scripts/with-env-local.sh -- env TELEGRAM_RISK_SCOPES=read,presence target/debug/telegramd

scripts/with-env-local.sh -- target/debug/telegram-cli --output json workflow describe inspect_chat
scripts/with-env-local.sh -- target/debug/telegram-cli --output json session hold read,presence 60000
scripts/with-env-local.sh -- target/debug/telegram-cli --output json workflow run LEASE_ID inspect_chat '{"target":{"kind":"id","chat_id":CHAT_ID_FROM_CHAT_001},"open":true}'
scripts/with-env-local.sh -- target/debug/telegram-cli --output json session release LEASE_ID
```

Ожидается `status=ok`, root/result `complete=true`, `used_open_lease=true`. Workflow возвращает
успех только после correlated `closeChat` `ok`. При timeout ответа `openChat` выполняется ровно
один exact compensating `closeChat`; full-info error/timeout также закрывает lease. Cleanup имеет
отдельное bounded окно в четыре секунды, поэтому исчерпанный workflow deadline не запрещает
получить acknowledgement. Timeout самого cleanup остаётся uncertain и не запускает blind retry.

Deterministic evidence покрывает четыре ветки: success pairing, TDLib full-info error с cleanup,
timeout ответа `openChat` с compensating close и full-info timeout с полученным cleanup ACK. После
каждой timeout-ветки acknowledgement компенсирующего/финального `closeChat` принят и не остаётся
unmatched; поздний response исходного timed-out request этим тестом не моделируется.

Live evidence 2026-07-21: returning authorization достигла `ready`; workflow обнаружен и описан,
`read,presence` lease выдан, безопасный public supergroup fixture выбран из main list без вывода
его ID/title. `inspect_chat(open=true)` вернул root/result `complete=true`,
`used_open_lease=true`, `full_info_kind=supergroup`; correlated cleanup получен до success.
Lease освобождён явно, daemon завершился `Draining -> Closed`. Join/send/message read не
выполнялись; fixture и raw TDLib response в evidence не сохранены.
...
scripts/with-env-local.sh -- target/debug/telegram-cli --output json workflow describe membership_status
scripts/with-env-local.sh -- target/debug/telegram-cli --output json workflow run LEASE_ID preview_invite_link '{"url":"DISPOSABLE_INVITE_LINK"}'
...
scripts/with-env-local.sh -- target/debug/telegram-cli --output json workflow run LEASE_ID membership_status '{"kind":"invite_link","url":"DISPOSABLE_INVITE_LINK"}'
scripts/with-env-local.sh -- target/debug/telegram-cli --output json session release LEASE_ID
```
...
receipt `submission_complete=true`; `request_pending` при этом сохраняет
`membership_complete=false` и не должен повторяться или обходиться через join по известному ID.
Позднее принятие доказывает только read-only `membership_status` с root/result `complete=true`,
`state=member` и тем же chat ID. `unresolved` остаётся partial, а не declined/not-found.

Historical evidence 2026-07-19 до async-status refactor: returning authorization достигла
`ready`; typed contracts обнаружены до
...
Async evidence 2026-07-19: current-worktree daemon под отдельным `read` lease обнаружил и
описал `membership_status`, затем по той же invite получил root/result `complete=true`,
`state=member`, `freshness=server_snapshot` и chat ID present. `ensure_membership` и join methods
не вызывались; lease released, daemon `Draining -> Closed`. Это закрывает поздний approval без
повторной мутации; fixture/title/chat ID/raw response в evidence не сохранены.
