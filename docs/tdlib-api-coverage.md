...
Статус: exact schema snapshot, strict Rust parser/inventory, reviewed 1010-method owner policy/artifact, bounded generator и macOS arm64 native artifact закреплены; Linux artifact, capability/risk/retry classification, full registry и runtime implementation ещё не созданы.
...
- Owner corpus: [`policy/tdlib-feature-owners.json`](../policy/tdlib-feature-owners.json) и [`generated/tdlib-feature-owners.json`](../generated/tdlib-feature-owners.json); read-only gate: `cargo run --locked --offline -p tdlib-registry-gen -- check`; package gate: `cargo test --locked --offline -p tdlib-registry-gen --jobs 2`.
...
Этот exact commit принят как initial production schema pin в `D-20260715-003`; переход на другой commit требует явного manifest/schema diff, parser gate и повторной классификации. Strict-subset parser/inventory принят в `D-20260715-006` и `W-20260715-009`; bounded owner generator — в `D-20260715-007` и `W-20260715-010`; exact owner corpus — в `D-20260715-008` и `W-20260715-011`. Owner corpus доказывает только единственного feature owner для каждого method: generated registry/codec/router, capability/risk/retry и runtime parity из него не следуют. Текущий macOS arm64 artifact и crash ownership доказаны correction checkpoint `W-20260715-008`; `W-20260715-007` остаётся историей первой pre-review build. Linux x86_64 artifact и classified generated registry остаются отдельными P0 gates. Одна reviewed macOS rebuild не считается доказательством bit-for-bit reproducibility; RSS/tree limits являются sampled watchdog thresholds.
...
Canonical owner-only output содержит schema/policy/generator evidence, per-feature counts/hashes и sorted exact method rows с signature, owner source и candidate set. Текущий corpus содержит 17 rules, 252 atoms и 372 exact overlap overrides; все 1010 parsed methods имеют ровно одного owner. Canonical mapping SHA-256 — `4687dc47c69e798e0afb9e0ea8ee82ed5d91bcde00e51f476ff08ea8226ce790`, независимый digest exact `method + NUL + feature_id + LF` — `72c741dc024091b5f25f5fc97a46224a557560dcb483bb0d3a29c4085f9b13ac`. Output намеренно не содержит фиктивные capability/risk/retry/codec/router значения: эти поля и full registry parity остаются следующими P0/P3 gates.
...
| F019 | statistics, async graphs, read-only revenue reporting, storage and network resources |
