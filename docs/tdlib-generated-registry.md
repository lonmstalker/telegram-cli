...
Registry и generated block `docs/tdlib-api-coverage.md` пересоздаются
`cargo run -p tdlib-registry-gen`. Gate `python3 scripts/check-tdlib-registry.py`
проверяет deterministic equality обоих committed artifacts и generator output;
schema identity по-прежнему закрепляет только
