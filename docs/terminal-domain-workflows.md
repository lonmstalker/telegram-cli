...
F014 переиспользует этот upload proof вместо второго media pipeline. Typed
`plan_custom_emoji_set`/`apply_custom_emoji_set` принимают `create|add|delete`; nested
TDJSON строится только внутри core. Create/add проверяют terminal uploaded file и exact
owner/set state, mutation response всегда подтверждается reread. Lost add response не
повторяется: exact file ищется в перечитанном inventory. Delete cleanup доказан только
освобождённым set name. Полный contract: [`sticker-set-workflow.md`](sticker-set-workflow.md).
