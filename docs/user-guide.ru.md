...
> Установленная версия: `telegram-cli setup` один раз в терминале владельца, затем
> `telegram-cli --agent login` / `telegram-cli --agent run ...`. Daemon запускается
> автоматически из сохранённого профиля. Установка, skill и быстрый старт — в
> [README](../README.md). Ниже сохранён также development flow через `.env.local`.

[English version](user-guide.en.md) · [Пошаговая авторизация](authorization-guide.ru.md)

## Статус руководства

Это руководство описывает запуск из исходного checkout. Локальный installer и lazy profile доступны; systemd/launchd и проверенные upgrades остаются в P9. Первая авторизация с phone/OTP/2FA и повторный запуск того же зашифрованного профиля проверены live; остальные сценарии P10 ещё не завершены.
