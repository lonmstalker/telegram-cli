...
- [x] Добавить явный owner flow `login phone` для перехода с незавершённого QR на номер/код без повторного API setup.
...
- [x] B5. Daemon resend хранит только timeout gate; availability проверяет core.
- [x] B6. Lifecycle и secure-login docs отражают actual interactive startup и single-owner timeout.
