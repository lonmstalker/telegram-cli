...
fn generate(root: &Path) -> Result<GeneratedArtifacts, Box<dyn Error>> {
    ...
    let mut output = String::from(concat!(
        ...
        "use super::{\n",
        "    AccountKind, CapabilityDescriptor, CapabilityDisposition, FieldDescriptor, RetryClass,\n",
        "    RiskClass, SchemaDescriptor, SymbolDescriptor, SymbolKind, TypeDescriptor,\n",
        "};\n\n",
        "#[rustfmt::skip]\n",
    ));
    writeln!(
...
fn coverage_block(
    ...
    writeln!(
        output,
        "Core equality: `schema_functions({schema_methods}) == registry_methods({registry_methods}) == core_raw_methods({registry_methods})`. CLI/MCP parity остаётся незакрытой до соответствующих phases; workflow/live levels считаются отдельно."
    )?;
    Ok(output)
}
